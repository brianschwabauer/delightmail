<script lang="ts" module>
	// Inlined Lucide (ISC-licensed) icon path data — a small, self-contained set so
	// the app has crisp SVG icons with no runtime icon-font/emoji dependency. Each
	// entry is the inner markup of a 24×24 stroke icon (fill none, currentColor).
...
<script lang="ts">
	interface Props {
		name: IconName;
		/** Edge length; a number is px, a string is used verbatim. @default '1em' */
		size?: number | string;
		/** Fill the shape (e.g. a selected star) instead of stroking only. */
		fill?: boolean;
		/** Stroke weight in user units of the 24×24 grid. @default 2 */
		stroke?: number;
		class?: string;
	}
	const { name, size = '1em', fill = false, stroke = 2, class: className = '' }: Props = $props();

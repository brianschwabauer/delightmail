/**
 * MD5 (RFC 1321) → lowercase hex, over the UTF-8 bytes of the input.
 *
 * The only reason this exists: Gravatar keys every avatar by the MD5 of the
 * lowercased, trimmed email, and Web Crypto (the platform hash we'd otherwise
 * reach for) doesn't offer MD5 and is async-only — awkward for a value we want
 * synchronously while rendering a row. This is NOT used for anything security-
 * sensitive; MD5 is used solely because it's the hash Gravatar's URL scheme
 * requires. Implementation is the classic 32-bit-word MD5 (cf. blueimp-md5).
 */
...
/** Core transform: `x` is the message as little-endian 32-bit words, `len` its bit length. */
function binlMD5(x: number[], len: number): number[] {
	...
	for (let i = 0; i < x.length; i += 16) {
		...
		// Every x[i + k] may be a padding hole (undefined) — coerce to 0.
		const w = (k: number): number => x[i + k] || 0;
